...
# content_filter override → ORIGINATING amavis port for DKIM signing
submission inet n       -       y       -       -       smtpd
  ...
  -o content_filter=smtp-amavis-orig:[127.0.0.1]:10026
...
MASTEREOF
    ...
    # Wire Amavis as content_filter (anti-spam, anti-virus, DKIM signing).
    # Disable by setting DISABLE_AMAVIS=YES in .env.
    if [ "${DISABLE_AMAVIS}" != "YES" ]; then
        postconf -e "content_filter = smtp-amavis:[127.0.0.1]:10024"

        if ! grep -q "^smtp-amavis " /etc/postfix/master.cf; then
            echo "Adding amavis transport to master.cf..."
            cat >> /etc/postfix/master.cf << 'AMAVISEOF'

# Amavis content filter — outbound to amavis (port 10024, inbound policy)
...
# Amavis ORIGINATING content filter — port 10026 (signs DKIM, skips spam check).
# Used by submission/smtps via -o content_filter=smtp-amavis-orig:[127.0.0.1]:10026
smtp-amavis-orig unix - - n - 2 smtp
  -o syslog_name=postfix/amavis-orig
  -o smtp_data_done_timeout=1200
  -o smtp_send_xforward_command=yes
  -o disable_dns_lookups=yes
  -o max_use=20

# Amavis re-injection listener — inbound from amavis (port 10025).
# Permissive: trust 127.0.0.1 only. Skip header checks to avoid double-scan.
...
configure_amavis() {
    if [ "${DISABLE_AMAVIS}" = "YES" ]; then
        echo "Amavis disabled by configuration"
        return 0
    fi

    echo "Configuring Amavis (spam/virus scanning + DKIM signing)..."

    # 1) Enable virus + spam checks. Debian's default 15-content_filter_mode
    #    keeps both @bypass_*_checks_maps blocks COMMENTED — meaning bypass is
    #    NOT active, but the maps aren't bound either, so checks are effectively
    #    OFF. Uncommenting binds the maps and turns checks ON.
    local cfm=/etc/amavis/conf.d/15-content_filter_mode
    sed -i 's|^#\(@bypass_virus_checks_maps\)|\1|;
            s|^#\(   \\%bypass_virus_checks,\)|\1|;
            s|^#\(@bypass_spam_checks_maps\)|\1|;
            s|^#\(   \\%bypass_spam_checks,\)|\1|' "$cfm"

    # 2) Override final destinies and SA scoring. We OWN this file (init.sh
    #    rewrites on every container start), so manual edits won't survive.
    local user_cfg=/etc/amavis/conf.d/50-user
    cat > "$user_cfg" << 'EOF'
# Auto-generated by init.sh — DO NOT EDIT by hand.
use strict;

# --- Spam / virus destinies -------------------------------------------------
# Pass tagged spam through; sieve moves it to Junk. (Default in 21-ubuntu_defaults
# is D_DISCARD which silently drops mail — bad for false positives.)
...
$sa_tag_level_deflt  = -999;     # always insert X-Spam-* headers (debuggable)
$sa_tag2_level_deflt = 5.0;      # add X-Spam-Flag: YES + subject prefix
$sa_kill_level_deflt = 9.0;      # would-discard threshold; D_PASS overrides
$sa_dsn_cutoff_level = 10;
$sa_spam_subject_tag = '[SPAM] ';

# --- Local domains ----------------------------------------------------------
# By default 05-domain_id sets @local_domains_acl = ( ".$mydomain" ) where
# $mydomain is from /etc/mailname (= mail.kirby.rocks). That doesn't cover any
# of our hosted domains (chiaruzzi.ch, kirby.rocks, maisonsoave.ch, …), so
# amavis treats inbound mail to those as "RelayedOpenRelay" and SKIPS spam
# header tagging. Extend the ACL with every domain that has a DKIM key.
EOF

    # Build @local_domains_acl from /var/lib/dkim/*.pem (one entry per domain).
    {
        echo -n "@local_domains_acl = ( \".\$mydomain\""
        if [ -d /var/lib/dkim ]; then
            for keyfile in /var/lib/dkim/*.pem; do
                [ -f "$keyfile" ] || continue
                local dom
...
# --- Originating port (DKIM signing) ----------------------------------------
# Listen on a second port (10026) for outbound mail so we can apply the
# ORIGINATING policy (originating=1 → DKIM signs). Postfix submission/smtps
# route here via `-o content_filter=smtp-amavis-orig:[127.0.0.1]:10026`.
...
EOF
    ...
    # 3) Append a dkim_key() line for every .pem in /var/lib/dkim/.
    #    Selector "dkim" matches what scripts/add-domain.sh prints to DNS.
    if [ -d /var/lib/dkim ]; then
        for keyfile in /var/lib/dkim/*.pem; do
            [ -f "$keyfile" ] || continue
            local dom
            dom="$(basename "$keyfile" .pem)"
            echo "dkim_key('${dom}', 'dkim', '${keyfile}');" >> "$user_cfg"
        done
    fi
...
EOF
    ...
    # 4) Add clamav user to amavis group so clamd can read amavis's per-message
    #    tmp dir (/var/lib/amavis/tmp/<msg>/parts). Without this the AV scan
    #    fails with "File path check failure: Permission denied" — amavis falls
    #    back gracefully but virus scanning is effectively off.
    if ! id -nG clamav 2>/dev/null | tr ' ' '\n' | grep -qx amavis; then
        usermod -aG amavis clamav
    fi
    # tmp dir g+rx so clamd (in amavis group) can traverse and read.
    mkdir -p /var/lib/amavis/tmp /var/lib/amavis/db /var/lib/amavis/virusmails
    ...
    if command -v sievec >/dev/null 2>&1; then
        for f in /etc/dovecot/sieve/before.d/*.sieve; do
            [ -f "$f" ] || continue
