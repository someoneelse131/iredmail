...
0. **Fallout aus dem Spam-Fix 2026-08-07 — drei offene Punkte.**
   - **`URIBL_BLOCKED` auf jeder Mail (grösster verbleibender Hebel).** `test.uribl.com.multi.uribl.com` → `"Query Refused ... [Your DNS IP: 212.227.222.195]"`. Der Host nutzt die geteilten IONOS-Resolver (`212.227.123.16/17`), URIBL sperrt geteilte Resolver. Damit sind die URI-Blocklisten tot, ausgerechnet das stärkste Signal für den aktuellen Link-Spam (Pharma/Health). Spamhaus ZEN + DBL antworten dagegen noch korrekt, das kann sich aber jederzeit ändern (Spamhaus sperrt geteilte Resolver ebenfalls). Fix: lokaler rekursiver Resolver (`unbound`) auf dem Host, `systemd-resolved` auf `DNS=127.0.0.1:5335` umbiegen. **Wichtig: NICHT über `dns:` in `docker-compose.yml`** — der Core-Container löst `db` über Dockers embedded DNS (127.0.0.11) auf, ein Override bricht die DB-Verbindung. Über den Host-Resolver zu gehen wirkt automatisch, weil 127.0.0.11 an den Host weiterreicht (`ExtServers: [host(127.0.0.53)]`).
   - **Bayes neu aufbauen.** DB am 2026-08-07 geleert (`sa-learn --clear`), Backup unter `data/amavis-spamassassin.bak-20260807`. Vorher: `nspam=17, nham=1285` — inert (SA braucht **200** Spam **und** 200 Ham, `bayes_min_spam_num`/`bayes_min_ham_num`) und vergiftet, weil der −3.72-Boden den Spam unter die Autolearn-Ham-Schwelle (0.1) drückte und er automatisch als Ham gelernt wurde. 0 von 6 gespeicherten Mails hatte je eine `BAYES_*`-Regel. Die 22 Junk-Mails wurden per `doveadm move` (umgeht `imap_sieve`, sonst hätte Regel 2 sie als **Ham** angelernt) zurück in die INBOX gelegt und die Keywords `$Junk`/`Junk`/`NonJunk` entfernt, damit der User sie neu anlernen kann. **Erwartungsmanagement: 22 Mails bringen Bayes nicht über die 200er-Schwelle**, das läuft sich über Wochen ein. Schwelle absenken wäre möglich, aber bei dünnem Korpus fehleranfällig — erst mal laufen lassen, das Scoring trägt jetzt selbst.
   - **ClamAV: `freshclam` läuft überhaupt nicht.** Es gibt keinen s6-Service dafür; die Signaturen standen seit dem ersten Image-Build (2026-01-18, DB 27884) still. Beim Rebuild am 2026-08-07 zog apt ClamAV **1.4.4 → 1.5.3**, und 1.5.3 lehnt die alten CVDs ab (`FIPS crypto hash limits enabled` + fehlende detached `.sign`-Dateien → `Can't verify database integrity`), clamd ist crash-geloopt. Sofort behoben mit einem manuellen `freshclam` (daily → 28084, `.sign`-Dateien nachgeladen), clamd läuft wieder. **Bleibt offen:** ohne Updater wiederholt sich das. Entweder Host-Cron (`docker exec --user clamav iredmail-core freshclam`, analog zu `scripts/sa-update.sh`) oder ein eigener s6-Service. Nebenbefund: `clamd.conf` hat `RELOAD` deaktiviert, freshclam kann clamd also nicht selbst zum Neuladen bewegen.
