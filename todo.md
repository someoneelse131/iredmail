...
- **SOGo memcached not running (P3, root-caused 2026-06-09).** `SOGoCache: SERVER HAS FAILED AND IS DISABLED UNTIL TIMED RETRY` floods `sogo.log` on every request. Confirmed cause: the `memcached` *daemon* is not installed in the image at all (only `libmemcached11` client lib present; no binary, no s6 service under `/etc/s6-overlay/s6-rc.d/`), but SOGo defaults to caching at `127.0.0.1:11211`. Non-fatal (preference/attribute caching only; sending now works via smtps fix e0870b9), but spams the log and loses the attribute cache. Fix: add memcached to the Dockerfile apt install + an s6 longrun service dir, OR explicitly disable SOGo caching. Recommend installing memcached (SOGo expects it; iRedMail stock ships it).
